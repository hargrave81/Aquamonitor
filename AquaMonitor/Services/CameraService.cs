...
namespace AquaMonitor.Web.Services
{
    ...
    public class CameraService: IHostedService, IDisposable
    {
        ...
        private Queue<string> frames = new Queue<string>();
        ...
        async private void DoWork(object state)
        {
            ...
            else
            {
                ...
                logger.LogInformation(
                    "Camera Service is busy on Count: {Count}", count);
            }
        }
        ...
        private async Task ProcessWork(object state)
        {
            try
            {
                ...
                var thumbNail = System.IO.Path.Combine(System.IO.Path.GetDirectoryName(Assembly.GetExecutingAssembly().Location),
